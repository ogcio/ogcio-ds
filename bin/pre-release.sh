...
if [[ -n $(git status --porcelain) ]]; then
  ...
  git commit -m "Release GOV.IE Frontend to '$BRANCH_NAME' for testing"
...
echo
echo "✅ Success! To install the pushed branch release, run 'npm install --save \"alphagov/govie-frontend#$BRANCH_COMMIT_SHA\"'"
