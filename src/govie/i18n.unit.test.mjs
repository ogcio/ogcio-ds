...
describe('I18n', () => {
  describe('.t', () => {
    ...
    beforeEach(() => {
      translations = {
        ...
        htmlString: 'Hello<span class="govie-visually-hidden"> world</span>'
      }
    })
    ...
    it('returns the HTML for a given lookup key', () => {
      ...
      expect(returnString).toBe('Hello<span class="govie-visually-hidden"> world</span>')
    })
  ...
  describe('.getPluralSuffix', () => {
    ...
    it.skip('uses `Intl.PluralRules` when available', () => {
      const IntlPluralRulesSelect = vi.spyOn(global.Intl.PluralRules.prototype, 'select')
        .mockReturnValue(() => 'one')
      ...
      expect(IntlPluralRulesSelect).toHaveBeenCalledOnce();
    })
