...
describe('Inset text', () => {
  ...
  describe('by default', () => {
    ...
    it('renders with classes', () => {
      ...
      const $component = $('.govie-inset-text')
    ...
    it('renders nested components using `call`', () => {
      ...
      expect($('.govie-inset-text .app-nested-component').length).toBeTruthy()
    })

    it('allows text to be passed whilst escaping HTML entities', () => {
      const $ = render('inset-text', examples['html as text'])

      const content = $('.govie-inset-text').html().trim()
    ...
    it('allows HTML to be passed un-escaped', () => {
      ...
      const mainContent = $('.govie-inset-text .govie-body:first-child').text().trim()
      const warningContent = $('.govie-inset-text .govie-warning-text__text').text().trim()
