...
describe('/examples/template-default', () => {
  describe('skip link', () => {
    ...
    it('adds the tabindex attribute to the linked element', async () => {
      const tabindex = await page.$eval('.govie-main-wrapper', el => el.getAttribute('tabindex'))

      expect(tabindex).toBe('-1')
    })

    it('adds the class for removing the native focus style to the linked element', async () => {
      const cssClass = await page.$eval('.govie-main-wrapper', el => el.classList.contains('govie-skip-link-focused-element'))

      expect(cssClass).toBeTruthy()
    })

    it('removes the tabindex attribute from the linked element on blur', async () => {
      await page.$eval('.govie-main-wrapper', el => el.blur())
    ...
    it('removes the class for removing the native focus style from the linked element on blur', async () => {
      ...
      const cssClass = await page.$eval('.govie-main-wrapper', el => el.getAttribute('class'))

      expect(cssClass).not.toContain('govie-skip-link-focused-element')
    })
  })
})
