...
describe('@mixin govie-compatibility', () => {
  ...
  it('does not output if the app is not marked as included', async () => {
    const sass = `
      ...
      $_govie-compatibility: (existing_app: false);

      @import "tools/compatibility";

      @include govie-compatibility(existing_app) {
        .foo {
          color: red;
        }
      }`
  ...
  it('outputs if the app is not marked as included', async () => {
    const sass = `
      ${warningsImport}
      $_govie-compatibility: (existing_app: true);
  ...
  it('throws an exception if the app is not recognised', async () => {
    const sass = `
      ...
      @include govie-compatibility(non_existent_app) {
        .foo {
          color: red;
        }
      }`
