...
describe('@function govie-colour', () => {
  ...
  describe('when $govie-use-legacy-palette is true', () => {
    beforeEach(() => {
      sassBootstrap = `
        @import "settings/warnings";
        $govie-use-legacy-palette: true;
        ${sassBootstrap}
      `
    })
    ...
    it('returns the legacy colour if specified', async () => {
      const sass = `
        ...
        .foo {
          color: govie-colour('red', $legacy: 'blue');
        }`
    ...
    it('returns the legacy literal if specified', async () => {
      const sass = `
        ...
        .foo {
          color: govie-colour('red', $legacy: #BADA55);
        }`
    ...
    it('does not error if the non-legacy colour does not exist', async () => {
      const sass = `
        ...
        .foo {
          color: govie-colour('hooloovoo', $legacy: 'blue');
        }`
    ...
    it('throws an error if the legacy colour does not exist', async () => {
      const sass = `
        ${sassBootstrap}
        .foo {
          color: govie-colour('red', $legacy: 'hooloovoo');
        }`
    ...
    it('outputs a deprecation warning when set to true', async () => {
      ...
      await renderSass({ data: sass, ...sassConfig }).then(() => {
        ...
        return expect(mockWarnFunction.mock.calls[0][0].getValue())
          .toEqual(
            '$govie-use-legacy-palette is deprecated. Only the modern colour ' +
            'palette will be supported from v5.0. To silence this warning, ' +
            'update $govie-suppressed-warnings with key: "legacy-palette"'
          )
      })
    })
  })

  describe('when $govie-use-legacy-palette is false', () => {
    beforeEach(() => {
      sassBootstrap = `
        $govie-use-legacy-palette: false;
        ${sassBootstrap}
      `
    })
