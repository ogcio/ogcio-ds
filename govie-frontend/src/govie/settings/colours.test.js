...
describe('Organisation colours', () => {
  it('should define websafe colours that meet contrast requirements', async () => {
    const sass = `
      ...
      @each $organisation in map-keys($govie-colours-organisations) {

        $colour: govie-organisation-colour($organisation);
        $contrast: ch-color-contrast($govie-body-background-colour, $colour);

        @if ($contrast < $minimum-contrast) {
          @error "Contrast ratio for #{$organisation} too low."
          + " #{$colour} on #{$govie-body-background-colour} has a contrast of: #{$contrast}."
