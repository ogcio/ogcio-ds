# Publish pre-release of GOV.IE Frontend
...
Before you publish a pre-release, you need to have committed a code change to GOV.IE Frontend. Then follow these instructions.
...
> :warning:Your projects should never depend on a pre-released GOV.IE Frontend package. This is because someone could remove the GitHub branch containing the pre-release package at any time. For this reason, never use a pre-released package in a production setting.

## What happens when you pre-release GOV.IE Frontend

When you pre-release GOV.IE Frontend, this creates a GitHub branch. This branch contains the GOV.IE Frontend `/package` directory with your trial changes.

Projects can point to this branch in their package.json, instead of to the published [GOV.IE Frontend npm package](https://www.npmjs.com/package/govuk-frontend). No changes are published to the GOV.IE Frontend npm package as part of this process.
...
2. Navigate to the project in the command line and run the success notification command. Running this command makes the project point to the pre-release branch, instead of to the published [GOV.IE Frontend npm package](https://www.npmjs.com/package/govuk-frontend). You can now preview your trial changes to GOV.IE Frontend.
