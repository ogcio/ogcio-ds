...
describe.skip('package/', () => {
  ...
  beforeAll(async () => {
    ...
    componentsFilesPackage = await getListing(
      join(configPaths.package, 'govie/components'),
    );
    componentsFilesPackageESM = await getListing(
      join(configPaths.package, 'govie-esm/components'),
    );
      ...
      .flatMap(
        mapPathTo(['**/*.mjs'], ({ dir: requirePath, name }) => {
          ...
          if (importFilter.test(requirePath)) {
            output.push(
              join(
                requirePath.replace(importFilter, 'govie-esm'),
                `${name}.mjs`,
              ),
            );
          }
  ...
  describe('README.md', () => {
    it('is not overwritten', async () => {
      const contents = await readFile(
        join(configPaths.package, 'README.md'),
        'utf8',
      );
  ...
  describe('all.js', () => {
    it('should have correct module name', async () => {
      const contents = await readFile(
        join(configPaths.package, 'govie', 'all.js'),
        'utf8',
      );
  ...
  describe('components with JavaScript', () => {
    ...
    beforeAll(async () => {
      ...
      componentNamesWithJavaScript = componentNames.filter((componentName) =>
        componentsFilesSource.includes(
      ...
      const componentTasks = componentNamesWithJavaScript.map(
        async (componentName) => {
          ...
          const componentPackage =
            componentsFilesPackage.filter(componentFilter);
          const componentPackageESM =
            componentsFilesPackageESM.filter(componentFilter);
          ...
          expect(componentPackage).toEqual(
          ...
          expect(componentsFilesPackageESM).toEqual(
            expect.arrayContaining([
              join(componentName, `${componentName}.mjs`),
            ]),
          );

          const [modulePath] = componentPackage.filter(
            filterPath([`**/${componentName}.js`]),
          );

          const [modulePathESM] = componentPackageESM.filter(
            filterPath([`**/${componentName}.mjs`]),
          );

          const moduleName = componentPathToModuleName(
            join(configPaths.components, modulePath),
          );
          const moduleText = await readFile(
            join(configPaths.package, 'govie/components', modulePath),
            'utf8',
          );
          const moduleTextESM = await readFile(
            join(configPaths.package, 'govie-esm/components', modulePathESM),
            'utf8',
          );
          ...
          expect(moduleTextESM).toContain(
            `export default ${componentNameToClassName(componentName)}`,
          );
        },
      );
